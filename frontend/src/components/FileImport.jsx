...
// Aici am creat componenta pentru încărcarea unui fișier .txt cu nume, folosind FileReader API.
// Formatul așteptat este un nume pe linie, iar liniile goale sunt ignorate automat
export default function FileImport({ onImport }) {
  // Aici am setat fileInputRef ca referință către input-ul ascuns
  // Am folosit un buton stilizat pentru un design uniform
  const fileInputRef = useRef(null);

  // Aici am stocat numele fișierului încărcat pentru a fi afișat ca feedback vizual
  const [fileName, setFileName] = useState("");

  // Aici am stocat mesajul de eroare local
  const [error, setError] = useState("");

  // Aici am setat starea de loading în timpul citirii asincrone a FileReader-ului
  const [loading, setLoading] = useState(false);

  // Aici am creat handler-ul la selectarea unui fișier
  function handleFileChange(e) {
    const file = e.target.files[0];

    // Aici am verificat dacă utilizatorul a anulat selecția
    if (!file) return;
    ...
    // Aici am folosit FileReader API pentru a citi fișierul în memoria browser-ului
  ...
  return (
    <div>
      ...
      <div className="flex gap-2">
        ...
        {/* Buton de reset care apare doar dacă există un fișier încărcat */}
        {fileName && !loading && (
          <button
            type="button"
            onClick={handleReset}
